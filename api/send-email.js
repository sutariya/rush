...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (formType === 'contact') {
      ...
      `;
    } else if (formType === 'unblock') {
      const cleanTxHash = txHash?.trim();
      const cleanNetwork = network?.trim();
      const cleanAmount = amount?.trim();
      const cleanTld = tld?.trim();

      if (!cleanTld || !cleanAmount || !cleanNetwork || !cleanTxHash) {
        return res.status(400).json({ error: 'TLD, amount, network, and transaction hash are required' });
      }
      ...
      emailSubject = `TLD Unblock Request: ${cleanTld} (${cleanAmount} ${cleanNetwork})`;
      emailHtml = `
        <h2>🔓 New TLD Unblock Request</h2>
        <ul>
          <li><strong>TLD:</strong> ${cleanTld}</li>
          <li><strong>Name:</strong> ${(name || 'Anonymous').trim()}</li>
          <li><strong>X Handle:</strong> ${(xHandle || '—').trim()}</li>
          <li><strong>Email:</strong> ${email.trim()}</li>
          <li><strong>Company:</strong> ${(company || '—').trim()}</li>
          <li><strong>Amount:</strong> ${cleanAmount} ${cleanNetwork}</li>
          <li><strong>Network:</strong> ${cleanNetwork}</li>
          <li><strong>Transaction:</strong> ${
            txLink
              ? `<a href="${txLink}" target="_blank" style="color: #00ffaa; text-decoration: none;">View on Explorer</a>`
        ...
        <p><em>Verify transaction and process TLD unblock request. Minimum: $1000 USD equivalent.</em></p>
      `;
    } else {
